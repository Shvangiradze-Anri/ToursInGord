...
function Carouserl() {
  ...
  const updateIndex = useCallback(
    (newIndex: number) => {
      if (newIndex < 0) {
        newIndex = 0;
      } else if (newIndex >= memoizedHotelImages.length) {
        newIndex = memoizedHotelImages.length - 1;
      }
      setActiveIndex(newIndex);
    },
    [memoizedHotelImages.length]
  );
      ...
      <div className="flex flex-col gap-y-2 justify-center rounded-2xl overflow-hidden w-full min-400:w-4/5 min-700:w-3/4 min-800:w-2/3 min-900:w-[35rem]">
        ...
        <div className="flex flex-col items-end">
          <div className="flex gap-x-4">
            ...
            <button
              onClick={() => updateIndex(activeIndex + 1)}
              disabled={activeIndex >= memoizedHotelImages.length - 1}
            >
              <svg
                className="w-6 h-fit min-500:w-8 min-1200:w-9 min-1400:w-10"
                viewBox="0 0 24 24"
              ...
              >
                <path
                  ...
                  fill={
                    activeIndex >= memoizedHotelImages.length - 1
                      ? "grey"
                      : "white"
                  }
                />
